...
MINUS
SELECT CUSTID FROM ORDERS; --커스터머의  id 데이터 중에서 오더스의 id 데이터와 같은 값은 제외하고 조회
-------
--(방법2) 서브쿼리(SUB QUERY)
SELECT *
FROM CUSTOMER
WHERE CUSTID NOT IN (SELECT CUSTID FROM ORDERS)
; --아까 실습 02_그룹바이에서 맨 마지막 문제 해메이던게 바로 이거군..!!!!!!!!!
-- 괄호랑 셀렉트를 다시 해줘야하는군.
-------
--(방법3) 외부조인(OUTER JOIN)
...
;
SELECT *
FROM DEPT2 D2 RIGHT OUTER JOIN DEPT1 D1 
     ON D1.ID = D2.ID 
WHERE D2.ID  IS NULL
;
...
;
SELECT *
FROM DEPT2 D2 LEFT OUTER JOIN DEPT1 D1
     ON D1.ID = D2.ID 
--WHERE D1.ID IS NULL
;
--------------
...
AND D2.ID IS NULL --찾을 조건(검색조건)
;
SELECT D1.*
FROM DEPT1 D1 LEFT OUTER JOIN DEPT2 D2 --조인테이블 + 조인방식(좌측기준)
     ON D1.ID = D2.ID --조인조건
WHERE D2.ID IS NULL --찾을 조건(검색조건)
;
--2. DEPT2 에는 있고, DEPT1 에는 없는 데이터 찾기(RIGHT OUTER JOIN)
SELECT D2.*
FROM DEPT1 D1, DEPT2 D2 --조인테이블
WHERE D1.ID(+) = D2.ID --조인조건(우측기준)
AND D1.ID IS NULL --찾을 조건(검색조건)
;
SELECT D2.*
FROM DEPT1 D1 RIGHT OUTER JOIN DEPT2 D2 --조인테이블 + 조인방식(우측기준)
     ON D1.ID = D2.ID --조인조건
WHERE D1.ID IS NULL --찾을 조건(검색조건)
;