...
; -- 오!!!!! 여기서는 괄호 빼도 됨!!!
-- 그리고 inner join 쓰면 꼭 저 뭐냐 별칭 써줘야하는 듯.
-- 맨 위에 문제1번?에 inner join 안쓴 코드랑 쓴 코드랑 별칭 유무 차이 있음.
--===========================
--썜-----------------------------------
SELECT * FROM BOOK;
SELECT * FROM ORDERS;
...
SELECT B.BOOKNAME, B.PUBLISHER, O.SALEPRICE, O.ORDERDATE
 ...
 WHERE B.BOOKID = O.BOOKID
   AND O.CUSTID = C.CUSTID
   AND C.NAME = '장미란'
   AND O.ORDERDATE >= TO_DATE('2014-01-01') AND O.ORDERDATE <= TO_DATE('2014-07-08')
;
SELECT B.BOOKNAME, B.PUBLISHER, O.SALEPRICE, O.ORDERDATE
  FROM ORDERS O INNER JOIN BOOK B
       ON O.BOOKID = B.BOOKID 
       ...
       ON O.CUSTID = C.CUSTID
 WHERE C.NAME = '장미란'   
 AND O.ORDERDATE BETWEEN TO_DATE('2014-01-01') AND TO_DATE('2014-07-08')
;  
...
SELECT '김하은', COUNT(*), SUM(O.SALEPRICE), AVG(O.SALEPRICE), MAX(O.SALEPRICE), MIN(O.SALEPRICE)
  FROM ORDERS O, BOOK B, CUSTOMER C
 WHERE B.BOOKID = O.BOOKID
   AND O.CUSTID = C.CUSTID
   AND C.NAME = '추신수'
...
;
SELECT '김하은' AS NAME, COUNT(*), SUM(O.SALEPRICE), AVG(O.SALEPRICE), MAX(O.SALEPRICE), MIN(O.SALEPRICE)
  FROM ORDERS O INNER JOIN BOOK B
       ON O.BOOKID = B.BOOKID 
       INNER JOIN CUSTOMER C
       ON O.CUSTID = C.CUSTID
...
WHERE O.CUSTID = C.CUSTID --조인조건
--AND C.NAME IN ('추신수', '장미란')
GROUP BY C.NAME -- 이게 그룹바이를 안해주면 오류네. 
--저 셀렉 옆 C.NAME 이 오류 안되려면 그룹바이 해줘야함.
;
